    ...
    with HOST_CSV.open("r", encoding="utf-8", newline="") as fh:
        ...
        for row in reader:
            ...
            if not host:
                continue
            label = name or host
            hosts.append({"host": host, "label": label, "hostname": name})
    return hosts
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Live STIG expert-critic web app")
    parser.add_argument("--mode", default="live", choices=("live",))
    parser.parse_args()
